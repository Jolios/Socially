...
$id_profil = $infoUtlConnecte['pseudo'];
$srcImg = "";
$result_path = "";
$msg = "";
...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'res/image/photos_de_profile/'.$id_profil.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/Projet/res/image/photos_de_profile';
	//Set File name	
		$file_temp_name = $id_profil.'_original.'.md5(time()).'n'.$type; //the temp file name
		$fullpath = $uploaddir."/".$file_temp_name; // the temp file path
		$file_name = $id_profil.'_temp.jpeg'; //$id_profil.'_temp.'.$type; // for the final resized image
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$srcImg= "res/image/photos_de_profile/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'res/image/photos_de_profile/'.$_POST['src'];	
		$finalname = $id_profil.md5(time());	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "res/image/photos_de_profile/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="res/image/photos_de_profile/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($con, "UPDATE utilisateur SET photo_de_profile='$result_path' WHERE pseudo='$utilisateurConnecte'");
		header("Location: profile.php?pseudo_profile=".$utilisateurConnecte);
...
<div class="colonne_principale colonne">
    ...
    <?php
    if($srcImg){ //if an image has been uploaded display cropping area?>
	    <script>
	    	$('#Overlay').show();
			$('#formExample').hide();
	    </script>
	    <div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; margin-left: -200px; position:relative; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0px;">  
	        ...
	        <div id="CroppingArea" style="width:500px; max-height:400px; position:relative; overflow:hidden; margin:40px 0px 40px 40px; border:2px #666 solid; float:left;">	
	            <img src="<?=$srcImg?>" border="0" id="jcrop_target" style="border:0px #990000 solid; position:relative; margin:0px 0px 0px 0px; padding:0px; " />
	        </div>  
